...
window.LESSONS.push({
  ...
  "teoria": [
    { type: 'callout', variant: 'tip', title: "L'idea in breve", content: "Dimmi con chi vai e ti diro chi sei. Per classificare un punto nuovo, guarda i K punti piu simili che gia conosci e vota a maggioranza. Nessun training vero: il modello e il dataset stesso." },
    ...
    {
      "type": "md",
      "content": "\n<h3>7.8 KNN per la regressione</h3>\n<p>La versione regressiva di KNN stima il valore target come <strong>media dei $k$ vicini</strong>:</p>\n<p>$$\\hat{y}(x) = \\frac{1}{k} \\sum_{i \\in \\mathcal{N}_k(x)} y_i$$</p>\n<p>Con <strong>pesatura inversa alla distanza</strong> ($w_i = 1/d(x, x_i)$):</p>\n<p>$$\\hat{y}(x) = \\frac{\\sum_{i \\in \\mathcal{N}_k(x)} w_i\\, y_i}{\\sum_{i \\in \\mathcal{N}_k(x)} w_i}$$</p>\n<p>I punti più vicini contribuiscono di più alla predizione. Caso degenere: se un vicino ha distanza 0 si usa direttamente il suo target. Valgono le stesse regole del classificatore: scaling obbligatorio, stessa maledizione della dimensionalità.</p>\n\n<h3>7.9 Confine di decisione e diagramma di Voronoi</h3>\n<p>Con $k=1$, KNN produce esattamente una <strong>partizione di Voronoi</strong> dello spazio delle feature: ogni cella contiene tutti i punti dello spazio che sono più vicini a un dato training point che a qualsiasi altro. Il confine di decisione coincide con i lati del diagramma di Voronoi tra punti di classi diverse.</p>\n<ul>\n<li><strong>$k=1$</strong>: confine frastagliato, forma arbitrariamente complessa — massima varianza</li>\n<li><strong>$k$ grande</strong>: il confine si ammorbidisce progressivamente, le celle si fondono — bias crescente</li>\n<li>La complessità è sempre <em>locale</em>: non esiste una forma globale parametrica come nel caso lineare</li>\n</ul>\n\n<h3>7.10 Scelta di $k$: bias-varianza e regole pratiche</h3>\n<p>La scelta di $k$ governa il trade-off bias-varianza:</p>\n<ul>\n<li>$k = 1$: massima varianza, overfit — accuracy training = 100% (ogni punto è il suo stesso vicino)</li>\n<li>$k = n$: massimo bias — predice sempre la classe maggioritaria del training set</li>\n<li><strong>Regola euristica di partenza</strong>: $k \\approx \\sqrt{n}$</li>\n<li>Il valore ottimale si trova sempre con <strong>cross-validation</strong></li>\n<li>Per classificazione <strong>binaria</strong>: preferire $k$ dispari per evitare pareggi nel voto di maggioranza</li>\n</ul>\n<p>Connessione con leave-one-out CV: con $k=1$, la leave-one-out CV misura direttamente la capacità di generalizzazione su ogni singolo punto (errore = 0 su training, quindi LOO CV è l'unica stima utile).</p>\n\n<h3>7.11 Metriche di distanza</h3>\n<p><strong>Minkowski $L_p$</strong> — famiglia generale che include le metriche più comuni:</p>\n<p>$$d_p(a,b) = \\left(\\sum_j |a_j - b_j|^p\\right)^{1/p}$$</p>\n<ul>\n<li>$p = 1$: <strong>Manhattan</strong> (distanza a blocchi, robusta agli outlier)</li>\n<li>$p = 2$: <strong>Euclidea</strong> (default, geometria standard)</li>\n<li>$p \\to \\infty$: <strong>Chebyshev</strong> — $d_{\\infty}(a,b) = \\max_j |a_j - b_j|$</li>\n</ul>\n<p><strong>Cosine similarity</strong> — misura l'angolo tra vettori, indipendente dalla magnitudine:</p>\n<p>$$\\cos(a,b) = \\frac{a \\cdot b}{\\|a\\|\\,\\|b\\|}, \\quad d_{\\cos}(a,b) = 1 - \\cos(a,b)$$</p>\n<p>Ideale per testi (TF-IDF, embeddings) dove conta la direzione tematica, non la lunghezza del documento.</p>\n<p><strong>Hamming</strong>: per vettori binari o categorici; conta il numero di posizioni in cui i due vettori differiscono. Usata per confronto di sequenze DNA, codici binari, feature categoriche one-hot.</p>\n<p>La scelta della metrica dipende dal dominio: dati sparsi/testuali → cosine; dati categorici → Hamming; dati continui densi → Euclidea o Manhattan.</p>\n\n<h3>7.12 Strutture dati per la ricerca efficiente</h3>\n<p>La ricerca naïve dei $k$ vicini ha complessità $O(nd)$ per ogni query ($n$ = punti training, $d$ = feature). Per dataset grandi si usano strutture specializzate:</p>\n<ul>\n<li><strong>Forza bruta</strong>: $O(nd)$ per query — semplice ma lento; con BLAS competitivo per $n$ piccolo</li>\n<li><strong>K-d tree</strong>: partiziona lo spazio in iperpiani allineati agli assi; complessità media $O(d \\log n)$; degrada verso $O(nd)$ per $d > 20$</li>\n<li><strong>Ball tree</strong>: usa sfere annidate invece di iperpiani; più stabile del k-d tree ad alta dimensione</li>\n<li><strong>Metodi approssimati</strong> (HNSW, LSH): per $n$ o $d$ enormi dove la soluzione esatta è impraticabile</li>\n</ul>\n<p>In scikit-learn: <code class=\"inline\">algorithm='auto'</code> sceglie automaticamente la struttura migliore; i valori espliciti sono <code class=\"inline\">'ball_tree'</code>, <code class=\"inline\">'kd_tree'</code>, <code class=\"inline\">'brute'</code>. Usa <code class=\"inline\">n_jobs=-1</code> per parallelizzare su tutti i core disponibili.</p>\n"
    },
    {
      "type": "callout",
      "variant": "tip",
      "title": "Regole pratiche per KNN",
      "content": "1) Scala sempre le feature (StandardScaler o MinMaxScaler) prima di calcolare distanze. 2) Usa k dispari per classificazione binaria. 3) Inizia con k ≈ √n e ottimizza con CV. 4) Per n > 10k, usa algorithm='ball_tree' o n_jobs=-1. 5) In alta dimensione (d > 20), applica PCA o UMAP prima di KNN per combattere la maledizione della dimensionalità."
    },
    {
      "type": "callout",
      "variant": "warn",
      "title": "Concentrazione della misura in alta dimensione",
      "content": "In alta dimensione le distanze euclideee si concentrano: (d_max - d_min) / d_min → 0 per d → ∞. Il risultato pratico è che tutti i punti sembrano equidistanti e i \"vicini\" perdono significato. Regola: se d > 50, KNN naïve è quasi sempre inutile senza riduzione dimensionale. La distanza cosine è più robusta in questo regime perché normalizza la magnitudine."
    }
  ],
  "esempi": [
    {
